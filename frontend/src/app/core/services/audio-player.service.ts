...
export class AudioPlayerService {
  ...
  private statusSignal = signal<string>('stopped');
  ...
  async play(track?: Track): Promise<void> {
    try {
      ...
      if (this.currentTrackSignal()) {
        ...
        // Incrémenter les lectures via l'API
        if (this.currentTrackSignal()) {
          this.trackApiService.incrementPlays(this.currentTrackSignal()!.id).subscribe();
        }
      }
    } catch (error) {
  ...
  private async loadTrack(track: Track): Promise<void> {
    ...
    try {
      ...
      // Utiliser l'URL de streaming de l'API
      const streamUrl = this.trackApiService.getAudioStreamUrl(track.id);
